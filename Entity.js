...
Player.onConnect = function (socket, username, progress) {
  ...
  for (var i in Player.list) {
    Player.list[i].socket.emit("PlayerJoin", player.username);
  }
  ...
  socket.on("sendMsgToServer", function (data) {
    for (var i in Player.list) {
      Player.list[i].socket.emit(
        "addToChat",
        "<b>" + player.username + "</b>" + ": " + data
      );
    }
  });
...
Player.onDisconnect = function (socket) {
  ...
  for (var i in Player.list) {
    Player.list[i].socket.emit("PlayerDisconnect", player.username);
  }
};
